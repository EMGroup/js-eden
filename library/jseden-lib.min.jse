...
func agentFromFile {
para name, file;
${{ edenUI.eden.agentFromFile(arguments[0],arguments[1]) }}$;
}
...
eden.error(new Error("distance: The first argument must be a Point or a Line, not a " + typeof(from)), "error");
return undefined;
}
} else {
eden.error(new Error("distance: This function requires at least 1 argument and at most 2 arguments"), "error");
return undefined;
}
if (to == undefined) {
return undefined;
} else if (!(to instanceof Point)) {
eden.error(new Error("distance: The destination point must be a Point, not a " + typeof(to)), "error");
return undefined;
}
