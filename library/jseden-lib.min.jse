...
eden.error(new Error("forgetAll: This function requires at most 3 arguments."), "error");
return [[], undefined, []];
}
...
var existingViewType = root.lookup("_view" + viewName + "_type").value();
...
eden.error(new Error("positionInList: The third argument must be a number, not a " + typeof(lowerBound)), "error");
return undefined;
...
var updateFunction = function (obs, value) {
CSSUtil.setStyle(cssRule, value);
};
var obsSym = root.lookup(obsName);
updateFunction("/" + obsName, obsSym.value());
obsSym.addJSObserver("bind", updateFunction);
...
_view_picture_x = max((screenWidth / 2 - 30) - _view_picture_width, _view_projects_width + _views_frame_width);
...
_view_input_x = min(screenWidth / 2 + 30, _view_picture_x + _view_picture_width + _views_frame_width + (_view_picture_x - (_view_projects_x + _view_projects_width + _views_frame_width)));
