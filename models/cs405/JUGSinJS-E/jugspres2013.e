...
s3 is Slide("<h3>Exploring the model</h3><p>To examine the model construction more closely, we can use the <b>Views</b> tab to create an instance of a <b>Symbol List</b>. </p>
<p>
...
s5 is Slide("<h3>Displaying observable values</h3><p>To display the observables relevant to a particular context, enter a regular expression into the search box at the top of the <b>Symbol List</b> (SL) panel.</p>
<p>
For instance, to focus on the key parameters in the JUGS model from a functional perspective, cut-and-paste the following RE into the search box and press Return:
</p>
<b>
...
<p>
The values - and definitions - of observables can be changed by directly entering a new definition via the Eden Interpreter Window. (In the emile prototype, it was also possible to edit definitions by double-clicking on the observable name in the <b>SL</b> panel, and editing and saving its definition as then displayed in a textbox, but this has yet to be implemented in the master variant.
</p>
<p>
...
<p>
The current values of these observables are displayed in the SL panel.
...
<p>
There was at one time a bug in the way in which the presentation handled the '&lt;' between the jseden tags, and this relation was formerly written as:
<jseden>Bfull is (max(capB,contentB)==contentB);</jseden>.
</p>
Workarounds of this nature are characteristic of the pragmatic spirit of EM, and are often useful when going beyond the capabilities of current tools.
");
...
<p>
The JUGS model brings out the contrast between the personal, concurrent and human-driven and the functional, sequential and automatic alluded to by SBR (cf. Session 2.3).
</p>
<p>
...
To see this, inspect the definition of the <code>pour</code> action by using the Views menu to bring up Joe Butler's <b>State Description Maintainer</b> (SDM) and ensuring that the <b>Actions</b> tab is highlighted.
<p>
You will see that <code>pour</code> acts like an agent that carries out automatically what the modeller might do manually.
For instance, when B is being filled (see Case 2 in the EDEN switch statement displayed in the SDM) the content of Jug B is incremented and the fullness of Jug B is checked repeatedly.
You can disclose this by intervening in the filling process by resetting <code>contentB</code> thus:
</p>
<p>
<jseden>contentB = 1;</jseden>
</p> 
- changing the viscosity observable to make intervention easier (see previous slide) if necessary.
");
