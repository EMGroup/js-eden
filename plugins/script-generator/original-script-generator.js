Eden.plugins.SG = function(context) {
	...
	SG.update = function(event){
		...
		for(var j=0; j<views.length; j++){
			...
			views[j].children[1].innerHTML = SG.generateBottomHTML(regex);
		}
	}
	
	SG.generateAllHTML = function(){
		//generates the regex
		var indiv = "<div class=\"SG\" style=\" display:block; \"><pre>"+SG.generateInnerHTML()+"</pre></div>";
		return indiv;
	}
	
	
	SG.generateInnerHTML = function(){
