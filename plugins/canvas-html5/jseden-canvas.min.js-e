...
if (viewTitle == @) {
titleSymName = "_view_" // viewName // "_title";
contentSymName = "_view_" // viewName // "_content";
execute(
titleSymName // " is \"Canvas \" // ${{ root.lookup(\"" // contentSymName // "\").eden_definition.match(/^" //
contentSymName // " is (.*)$/)[1] }}$;"
);
addJSObserver = ${{
function (contentSymName, titleSymName) {
root.lookup(contentSymName).addJSObserver("updateTitle", function () {
root.expireSymbol(root.lookup(titleSymName));
});
}
}}$;
addJSObserver(contentSymName, titleSymName);
} else {
