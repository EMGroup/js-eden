...
if (viewTitle == @) {
execute("_view_" // viewName // "_title is \"Canvas \" // _view_" // viewName // "_observable;");
} else {
...
if (name === undefined) {
eden.error(new Error("Button: Every button must a name."), "error");
return undefined;
}
label = arguments[0];
argsProcessed = 1;
...
return new Button(name, label, x, y, width, minHeight, enabled, fillcolour, textcolour);
}}$;
}

${{
Button = function (name, label, x, y, width, minHeight, enabled, fillcolour, textcolour) {
this.name = name;
...
argsProcessed++;
var labels = arguments[argsProcessed];
argsProcessed++;
var x = arguments[argsProcessed];
argsProcessed++;
var y = arguments[argsProcessed];
...
this.obsName = root.currentObservableName();
this.data = ptrToData? ptrToData.value() : [];
this.x = x;
this.y = y;
this.width = width;
...
if (this.drawingOptions !== undefined) {
var scale = (this.drawingOptions.lineWidth || edenUI.plugins.Canvas2D.defaultLineWidth) / context.lineWidth;
...
eden.error(new Error("angleTurned: This function requires at least 1 argument and at most 2 arguments"), "error");
return undefined;
}
if (to === undefined) {
return undefined;
...
return Math.atan2(relativeY, relativeX) / root.lookup("radiansPerUnit").value();
}}$;
}
radiansPerUnit ~> [angleTurned];
...
if (this.name == this.obsName) {
return "Slider(" + Eden.edenCodeForValues(this.min, this.max, this.step, this.x,
this.y, this.length, this.orientation, this.enabled) + ")";
} else {
return "Slider(" + Eden.edenCodeForValues(this.name, this.min, this.max, this.step, this.x,
this.y, this.length, this.orientation, this.enabled) + ")";
}
};
...
var rotation2;
if (n % 4 == 0) {
rotation2 = -rotation - Math.PI * (0.5 - 1 / n);
} else {
rotation2 = -rotation - Math.PI * (0.5 - 2 / n); //Zero rotation means "pointing upwards"
}
for (var i = 0; i < n; i++) {
var j = i;
...
var rotation;
if (this.n % 4 == 0) {
rotation = -this.rotation - Math.PI * (0.5 - 1 / this.n);
} else {
rotation = -this.rotation - Math.PI * (0.5 - 2 / this.n); //Zero rotation means "pointing upwards"
}
var nudge;
...
argsProcessed = 0;
if (name === undefined) {
eden.error(new Error("Textbox: Every text box have a name."), "error");
return undefined;
}
}
...
var preload = arguments[argsProcessed];
