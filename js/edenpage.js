	...
	if (page == '#eden-input' && cclass == 'code') {
		$('.exec', page).each(function() {
			...
			$area.bind('keyup','alt+n', function() {
				edeninput.setValue(eden.nextHistory());
			});
			$area.bind('keyup','alt+p', function() {
				edeninput.setValue(eden.previousHistory());
			});
			...
			if ($code.hasClass('eden')) {
				$button = $('<button>Submit</button>').click(function() {
					...
					try {
						eden.addHistory(edeninput.getValue());
						eval(Eden.translateToJavaScript(edeninput.getValue()));
						edeninput.setValue("");
						//printSymbolTable();
						printAllUpdates();
