...
Eden.DB = {
	...
	repositories: [
		"https://jseden.dcs.warwick.ac.uk/construalmanager",
		"http://localhost:18882"
	],
	repoindex: 0,
	retrycount: 0,
	connected: false,
	searchServer: "https://jseden.dcs.warwick.ac.uk",
	querycache: null,
	qcacheloaded: false,
...
Eden.DB.localSave = function(project) {
	...
	Eden.DB.setStorageItem(prefix+"_project", project.generate());
	Eden.DB.setStorageItem(prefix+"_id", project.id);
	Eden.DB.setStorageItem(prefix+"_vid", project.vid);
	Eden.DB.setStorageItem(prefix+"_author", project.author);
	Eden.DB.setStorageItem(prefix+"_authorid", project.authorid);
	Eden.DB.setStorageItem(prefix+"_name", project.name);
	Eden.DB.setStorageItem(prefix+"_title", project.title);
	Eden.DB.setStorageItem(prefix+"_thumb", project.thumb);
	Eden.DB.setStorageItem(prefix+"_desc", project.desc);
	Eden.DB.setStorageItem(prefix+"_tags", project.tags.join(" "));

	var plist = JSON.parse(Eden.DB.getStorageItem("project_list"));
	if (plist === null) plist = {};
	plist[project.id] = true;
	Eden.DB.setStorageItem("project_list", JSON.stringify(plist));
}
...
Eden.DB.loadLocal = function(id) {
	if (window && window.localStorage) {
		...
		var src = Eden.DB.getStorageItem(prefix+"_project");
		var id = Eden.DB.getStorageItem(prefix+"_id");
		if (id == "undefined") id = undefined;
		var vid = Eden.DB.getStorageItem(prefix+"_vid");
		if (vid == "undefined") vid = undefined;
		var author = Eden.DB.getStorageItem(prefix+"_author");
		if (author == "undefined") author = undefined;
		var authorid = Eden.DB.getStorageItem(prefix+"_authorid");
		var name = Eden.DB.getStorageItem(prefix+"_name");
		var tags = Eden.DB.getStorageItem(prefix+"_tags");
		var thumb = Eden.DB.getStorageItem(prefix+"_thumb");
		if (thumb == "undefined") thumb = undefined;
		var desc = Eden.DB.getStorageItem(prefix+"_desc");
		var title = Eden.DB.getStorageItem(prefix+"_title");
		if (src && src != "") {
			return {
				source: src,
				saveID: vid,
...
Eden.DB.hasLocal = function(pid, vid) {
	...
	if (plist !== null && plist[pid]) {
		...
		var lvid = Eden.DB.getStorageItem(prefix+"_vid");
			...
			success: function(data){
				if (data === null || data.error) {
					...
					if (!Eden.DB.qcacheloaded) {
						Eden.DB.querycache = JSON.parse(Eden.DB.getStorageItem("query_cache"));
...
Eden.DB.requestQCacheSave = function() {
	...
	Eden.DB.qcachetimeout = setTimeout(function() {
		Eden.DB.setStorageItem("query_cache", JSON.stringify(Eden.DB.querycache));
...
var browserID = Eden.DB.getStorageItem("construit_uid");
if (browserID === null || browserID === undefined || browserID == "") {
	browserID = guid();
	Eden.DB.setStorageItem("construit_uid", browserID);
}
var sessionID = guid();
